...
public class PluginLoader implements AutoCloseable {
    ...
    private void loadPluginComponents()
        ...
        try {
            ...
            List<Object> detectorNodeList = XMLUtil.selectNodes(pluginDescriptor, "/FindbugsPlugin/Detector");
            detectorNodeList.addAll(DetectorFindbugsHandle.get());
            int detectorCount = 0;
            for (Object detectorNode : detectorNodeList) {
                String className,speed,reports,requireJRE;
                boolean disabled, hidden;
                if (detectorNode instanceof Node) {
                    Node node = (Node) detectorNode;
                    className = node.valueOf("@class");
                    speed = node.valueOf("@speed");
                    disabled = "true".equals(node.valueOf("@disabled"));
                    reports = node.valueOf("@reports");
                    requireJRE = node.valueOf("@requirejre");
                    hidden = Boolean.valueOf(node.valueOf("@hidden")).booleanValue();
                } else if (detectorNode instanceof DetectorModel) {
                    DetectorModel d = (DetectorModel)detectorNode;
                    className = d.getClazz();
                    speed = d.getSpeed();
                    disabled = d.isDisabled();
                    reports = d.getReports();
                    requireJRE = d.getRequirejre();
                    hidden = d.isHidden();
                } else {
                    throw new PluginException("Missing @id for " + plugin.getPluginId()
                            + " loaded from " + loadedFrom);
                ...
                DetectorFactory factory = new DetectorFactory(plugin, className, detectorClass, !disabled, speed,
                        reports, requireJRE);
                if (hidden) {
        ...
        for (Object bugPatternNode1 : bugPatternNodeList) {
            ...
            int cweid;
            if (bugPatternNode1 instanceof Node) {
                Node bugPatternNode = (Node) bugPatternNode1;
                type = bugPatternNode.valueOf("@type");
                abbrev = bugPatternNode.valueOf("@abbrev");
                category = bugPatternNode.valueOf("@category");
                experimental = Boolean.parseBoolean(bugPatternNode.valueOf("@experimental"));
                cweid = 0;
                try {
                ...
                try {
                    ...
                    deprecated = deprecatedStr.length() > 0 && Boolean.valueOf(deprecatedStr).booleanValue();
                } catch (RuntimeException e) {
                    assert true; // ignore
                }
            } else if (bugPatternNode1 instanceof BugPatternFindbugs) {
                BugPatternFindbugs t = (BugPatternFindbugs) bugPatternNode1;
                type = t.type();
                abbrev = t.abbrev();
                category = t.category();
